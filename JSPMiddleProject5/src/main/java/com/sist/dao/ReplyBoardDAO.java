...
public class ReplyBoardDAO {
	...
	{	
		...
		try {
			...
			ps.setInt(1, no);
			ps.executeUpdate();
			sql="SELECT no,subject,name,regdate,content,hit FROM replyBoard WHERE no=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, no);
			ResultSet rs=ps.executeQuery();
			rs.next();
	...
	public void boardInsert(ReplyBoardVO vo)
	{
		try {
			...
			getConnection();
			String sql="INSERT INTO replyBoard(no,name,subject,content,pwd,group_id) VALUES(rb_no_seq.nextval,?,?,?,?,"
	...
	{	
		...
		try {
			...
			getConnection();
			String sql="SELECT no,subject,name,regdate,content,hit FROM replyBoard WHERE no=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, no);
			ResultSet rs=ps.executeQuery();
			rs.next();
		...
		try {
			...
			if(db_pwd.equals(pwd))
			{
				...
				else
				{
					...
					sql="UPDATE replyBoard SET subject=?,content=? WHERE no=?";
					ps=conn.prepareStatement(sql);
					ps.setString(1, msg);
					ps.setString(2, msg);
					ps.setInt(3, no);
		...
		//depth 증가
			try {
				...
				sql="UPDATE replyBoard SET "
						+ "group_step=group_step+1 WHERE group_id=? AND group_step>?"; //답변형의 핵심 SQL
				ps=conn.prepareStatement(sql);
				ps.setInt(1, gi);
				ps.setInt(2, gs);
				ps.executeUpdate();
	...
	public int boardFindCount(String fs,String ss)
	{
		int count=0;
			try {
				...
				getConnection();
				String sql="SELECT COUNT(*) FROM replyBoard WHERE "+fs+" LIKE '%'||?||'%'"; 
				ps=conn.prepareStatement(sql);
				ps.setString(1, ss);
				ResultSet rs=ps.executeQuery();
				rs.next();
	...
	public List<ReplyBoardVO> boardFindData(String fs,String ss)
	{
		List<ReplyBoardVO> list=new ArrayList<ReplyBoardVO>();
			try {
				...
				getConnection();
				String sql="SELECT no,subject,name,regdate,hit FROM replyBoard WHERE "+fs+" LIKE '%'||?||'%'"; 
				ps=conn.prepareStatement(sql);
				ps.setString(1, ss);
				ResultSet rs=ps.executeQuery();
				while(rs.next())
