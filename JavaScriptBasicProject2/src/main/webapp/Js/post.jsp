<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <%
 	...
 	if(dong!=null)
 	{
 		count=dao.postfindCount(dong);    // 처음은 NULL값을 받기 때문에 IF문으로 처리
    ...
    <%--
    	...
    	1-1) 스크립트릿
    		...
    		**2) 내장객체
    			...
    			***= out :
    			***= pageContext => 페이지 흐름
    				<jsp:include> <jsp:forward> => JSP액션택로 변경
    			= config (web,xml)
    			***= exception (try~catch
    			***= page(this)
    		3) JSP 액션 태그
    				<jsp:useBean> : 클래스 객체 생성
    				<jsp:setProperty> : set매소드에 값을 채운다
    				<jsp:getPropperty> : get메소드 호출
    				<jsp:param> : 데이터 전송
    				<jsp:include> : 특정 위치에서 다른 JSP를 추가
    				<jso: forward> : 화면 이동
    					= sendRedirect() => request를 초기화
    					=forward() => request르르 만들어 준다
    		**4) JSTL /EL
