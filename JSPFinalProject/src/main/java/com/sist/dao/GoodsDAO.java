...
public class GoodsDAO {
	...
	public List<GoodsVO> goodsListData(int page,int type)
	{
		...
		try {
			...
			String sql="SELECT no,goods_poster,goods_price,goods_name,num FROM (SELECT no,goods_poster,goods_price,goods_name,rownum as num "
					+ "FROM (SELECT no,goods_poster,goods_price,goods_name FROM "+tab[type]+" ORDER BY no ASC)) "
	...
	public GoodsVO goodsDetailData(int no,int type)
	{
		...
		try {
			...
			sql="SELECT no,goods_discount,account,goods_name,goods_sub,goods_price,goods_first_price,goods_delivery,goods_poster"
			...
			temp=temp.replaceAll("[^0-9]",""); //숫자를 제외한 나머지를 제거
