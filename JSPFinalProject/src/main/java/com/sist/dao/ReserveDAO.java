...
public class ReserveDAO {
   ...
   public void reserve_ok(ReserveVO vo)
   {
	   ...
	   {
		   conn=db.getConnection();
		   String sql="INSERT INTO reserve_info VALUES(ri_no_seq.nextval,?,?,?,?,?,'n',SYSDATE)";
		   ps=conn.prepareStatement(sql);
	   ...
	   List<ReserveVO> list=new ArrayList<ReserveVO>();
		try 
		{
			...
			String sql="SELECT no,fno,rday,rtime,inwon,TO_CHAR(regdate,'yyyy-MM-dd hh24:mi:ss'),foodGetPoster(fno),foodGetName(fno),"
					+ "foodGetPhone(fno),rok,id FROM reserve_info WHERE id=? ORDER BY no DESC";
			ps=conn.prepareStatement(sql);
			ps.setString(1, id);
			ResultSet rs=ps.executeQuery();
			while(rs.next())
   ...
   public void reserveOK(int no)
   {
	   try {
		...
		conn=db.getConnection();
		String sql="UPDATE reserve_info SET rok='y' WHERE no=?";
		ps=conn.prepareStatement(sql);
		ps.setInt(1, no);
		ps.executeUpdate();
	   } catch (Exception e) {
   ...
   public ReserveVO reserveInfoData(int no)
   {
	   ...
	   try {
		...
		conn=db.getConnection();
		String sql="SELECT no,rday,rtime,inwon,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') FROM reserve_info WHERE no=?";
		ps=conn.prepareStatement(sql);
		ps.setInt(1, no);
		ResultSet rs=ps.executeQuery();
		rs.next();
   ...
   //맛집정보
   public FoodVO reserveFoodInfoData(int no)
   {
	   FoodVO vo=new FoodVO();
	   try {
		   
		conn=db.getConnection();
		String sql="SELECT name,poster,address,phone,type,price,parking,menu,score FROM food_house WHERE fno=?";
		ps=conn.prepareStatement(sql);
		ps.setInt(1, no);
		ResultSet rs=ps.executeQuery();
		rs.next();
