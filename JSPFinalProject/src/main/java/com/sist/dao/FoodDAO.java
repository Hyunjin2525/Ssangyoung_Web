...
public class FoodDAO {
	...
	// 2. 실제 맛집 목록
		 ...
		 * CREATE OR REPLACE FUNCTION foodReplyData(
			...
			BEGIN
			    ...
			    FROM (SELECT msg,no FROM reply_all ORDER BY no DESC)
			    WHERE rownum=1;
	...
	//맛집 => 인근 명소 => 레시피
		public List<FoodVO> foodTop7()
		{
			...
			{
				conn=db.getConnection();
				String sql="SELECT fno,name,hit,rownum FROM (SELECT fno,name,hit FROM food_house ORDER BY hit DESC) WHERE rownum<=7";
				ps=conn.prepareStatement(sql);
				ResultSet rs=ps.executeQuery();
				while(rs.next())
				{
		...
		public List<RecipeVO> foodRecipeData(String type)
		{
			...
			try {
				conn=db.getConnection();
				String sql="SELECT no,title,chef,poster,rownum FROM recipe WHERE REGEXP_LIKE(title,?) AND rownum<=5"; //like문장이 많을 때는 REGEXP_LIKE()
				ps=conn.prepareStatement(sql);
				ps.setString(1, type);
				ResultSet rs=ps.executeQuery();
				while (rs.next()) 
