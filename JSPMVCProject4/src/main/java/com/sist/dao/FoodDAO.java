...
public class FoodDAO {
	...
	public List<FoodVO> foodCategoryListData(int cno)
	{
		...
		try {
			//1.오라클 연결
			getConnection();
			String sql="SELECT fno,name,score,poster,address,phone,type FROM food_house WHERE cno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, cno);
			ResultSet rs=ps.executeQuery();
			while(rs.next())
	...
	public FoodVO foodatailData(int fno)
	{
		...
		try {
			//1.오라클 연결
			getConnection();
			String sql="SELECT fno,cno,name,score,poster,address,type,parking,menu,phone,price FROM food_house WHERE fno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, fno);
			ResultSet rs=ps.executeQuery();
			rs.next();
