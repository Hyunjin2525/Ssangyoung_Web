...
<%--
	...
	2) 스크립트 : 자바/HTML을 분리해서 소스코딩
		...
		<%! %>: 선언문 (맨버변수,메소드 선언)
				=> 쿨랴스 제작시 클래스 블록 => 사용빈도가 없다
				=> 자바클래스를 만들어서 메소드를 호출
		<%	%>: 일번 메소드 영역 => 지역변수, 제어문, 메소드 호출 ...
	...
	3) 내장 객체 : 미리 객체를 생성해서 필요한 위치에서 사용이 가능
		request : HttpServletRequest
					...
					= 추가정보 (MVC,MV) => 오라클에서 받은 값을 추가 => JSP
						* setAttribite() : 기존에 있는 request값에 출력에 필요한 데아터를 추가해서 전송
						* getAttribute() : 추가해서 보낸 데이터를 받을 경우에 사용
						---------------- session에도 있다
					= 브라우저 정보
					   * getRemoteAddr() : 접속자의 IP
					   * getRequestURL(), getRequestURL(), getContextPath()
