...
public class EmpDAO {
	...
	public void empInsert(EmpVO vo)
	{
		try {
			...
			String sql="INSERT INTO myEmp VALUES((SELECT NVL(MAX(empno)+1,7000) FROM myEmp),?,?,?,SYSDATE,?,?,?)";
	...
	public List<EmpVO> empListData()
	{
		...
		try {
			...
			while(rs.next())
			{
				...
				vo.getSco().setGrade(rs.getInt(9));
				list.add(vo);
			}
			rs.close();
		} catch (Exception e) {
	...
	public EmpVO empDetailData(int empno)
	{
		...
		try {
			conn=db.getConnection();
			//2번째 문제 => 스칼라 서브쿼리로 변경
			String sql="SELECT empno,ename,job,NVL(mgr,0),TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),(SELECT dname FROM MYDEPT md WHERE me.deptno=md.deptno),(SELECT loc FROM MYDEPT md WHERE me.deptno=md.deptno),(SELECT grade FROM myGrade WHERE sal BETWEEN losal AND hisal)"
					+ " FROM myEmp me"
					+ " WHERE empno=?";
			ps=conn.prepareStatement(sql);
			//?에 값을 채운다
			...
			vo.getSco().setGrade(rs.getInt(10));
			rs.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
