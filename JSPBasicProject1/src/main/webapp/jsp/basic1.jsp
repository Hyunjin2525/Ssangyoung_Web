<%-- 주석 
	...
	3. Servlet VS JSP
		...
		Servlet
		JSP
		----------------> 웹서비스 기능 (사용자가 요청 => HTML로 변환해서 화면 추력)
	...
	5. JSP 문법
		...
		***= MVC
		...
		JSP => 24page
		------------
		동적 콘테츠 => 한개의 파일안에서 데이터를 변경할 수 있다
		자바언어로 사용된다
		자바 / HTML이 분리되어 있다
		JSP는 항상 => 서블릿으로 변경후에 실행
					------------------ 톰켓에 의해 변경
		JSP는 화면 출력할 내용의 메소드안에 들어가는 내용을 코딩한다
		---- 클래스가 아니고 메소드 영역이다
		_jspService()
		{
			-----------
	...
	_jspInit() ; web.xml => 저장된 내용 읽어서 저장
		|		erroro코드. 서블릿 저장 , 환경 설정 위치, 한글 변환
	_jspService() : 사용자 요청처리 결과
		|
