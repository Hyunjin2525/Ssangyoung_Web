  ...
  $(function(){ //window.onload=()=>{}
  	...
  	$('#logBtn').on('click',function(){
  		...
  		$.ajax({
  			...
  			url:'login_ok.jsp',
  			data:{"id":id,"pwd":pwd},
  			success:function(result) <%--collback 자동 호출--%>   <%--<%=vo.getMsg() %>--%>
  			{
  				let res=result.trim();
  				if(res=='NOID')
  					{
...
<body>
	<div class="container">
		<h1>Login</h1>
		<div class="row">
			<form method="post" action="login.ok.jsp" id="frm">
			<table class="table">
				<tr>
					<td width=30%>ID</td>
					<td width=80%>
						<input type=text name=id size=15 class="input-sm" id=id placeholder="아이디">
					</td>
				</tr>
				<tr>
					<td width=20%>Password</td>
					<td width=80%>
						<input type=password name=pwd  size=15 class="input-sm" id=pwd placeholder="비밀번호">
					</td>
				</tr>
				<tr>
