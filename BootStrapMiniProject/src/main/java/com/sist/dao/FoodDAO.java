...
public class FoodDAO {
	...
	public List<FoodVO> food_category_data(int cno)
	{
		...
		{
			getConnection();
			String sql="SELECT fno,name,poster,address,phone,type,score FROM food_house WHERE cno="+cno;
			ps=conn.prepareStatement(sql);
			ResultSet rs=ps.executeQuery();
			while(rs.next())
			{
	...
	public List<FoodVO> foodFindData(String addr, int page)
	{
		...
		{
			getConnection();
			//String sql="SELECT fno,name,poster,score FROM food_location WHERE address LIKE '%'||?||'%'";
			// =>mysql => LIKE CONCAT('%',?.'%')
			String sql="SELECT fno,name,poster,score,num FROM (SELECT fno,name,poster,score,rownum as num "
